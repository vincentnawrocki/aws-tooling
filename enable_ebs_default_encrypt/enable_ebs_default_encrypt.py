...
def enable_ebs_default_encryption(role: str, account_file: str):
    ...
    log.info(f"Default ebs encryption will be activated on all regions for the list of account(s): {account_list['accounts']}")

    for account in tqdm.tqdm(account_list['accounts'], desc="Accounts"):
        role_arn = f"arn:aws:iam::{account}:role/{role}"

        try:
            assume_role = sts_client.assume_role(RoleArn=role_arn, RoleSessionName="get_all_regions", DurationSeconds=3600)
            ...
            log.error(f"Failed to assume role during regions retrieval {role_arn} : {error}")
        ...
        for region in tqdm.tqdm(aws_regions, desc="Regions"):
            try:
                assume_role = sts_client.assume_role(RoleArn=role_arn, RoleSessionName=f"enable_ebs_encryption_{region}")
                session = boto3.Session(
                    aws_access_key_id=assume_role['Credentials']['AccessKeyId'],
                    aws_secret_access_key=assume_role['Credentials']['SecretAccessKey'],
                    aws_session_token=assume_role['Credentials']['SessionToken'],
    ...
    # Print error list
    if failure_list:
        log.error(f"Activation of EBS default encryption failed on account/region: {failure_list}")
    else:
